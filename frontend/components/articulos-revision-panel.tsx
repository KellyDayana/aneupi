  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-[#003952]">Revisión de Artículos</h2>
          <p className="text-sm text-gray-500 mt-0.5">Gestiona los artículos enviados por los usuarios</p>
        </div>
        <button
      ...
      <div className="flex flex-wrap gap-2">
        {(["PENDIENTE_APROBACION", "PUBLICADO", "RECHAZADO", "OCULTO", "TODOS"] as FiltroEstado[]).map((estado) => (
          ...
          >
            {estado === "PENDIENTE_APROBACION" ? "Pendientes" :
             estado === "PUBLICADO" ? "Publicados" :
             estado === "RECHAZADO" ? "Rechazados" :
             estado === "OCULTO" ? "Ocultos" : "Todos"}
          </button>
        ))}
      </div>
