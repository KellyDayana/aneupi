...
export default function AneupiTV() {
  ...
  useEffect(() => {
    // Esto solo se ejecuta en el navegador, después de la hidratación
  ...
  // Comentarios del artículo destacado
  ...
  useEffect(() => {
    if (pinnedFeatured) setFeaturedCollapsed(false)
  }, [pinnedFeatured])

  useEffect(() => {
    if (featuredCollapsed) setCommentsCollapsed(true)
  }, [featuredCollapsed])
  ...
  // Buscardor avanzado con filtros
  ...
  // --- CARRUSEL PRINCIPAL ---
  const [featuredSlides] = useState([
    {
      id: "f1",
      title: "Terrorismo en Ecuador | 19 coches bomba detectados",
      description: "Últimos cuatro casos reportados en menos de tres semanas en Guayaquil.",
    ...
    {
      id: "f2",
      title: "Cumbre Regional: acuerdos comerciales",
      description: "Líderes discuten nuevos acuerdos para el comercio regional.",
      category: "ECONOMÍA",
      date: "15 Oct 2025",
      image: "/sports-stadium-live.jpg",
      country: "Colombia",
    },
    {
      id: "f3",
    ...
    {
      id: "f5",
      title: "Salud pública: nueva campaña de vacunación",
      description: "Campaña masiva para reforzar la inmunización en zonas rurales.",
      category: "SALUD",
      date: "12 Oct 2025",
    ...
    {
      id: "f6",
      title: "Tecnología: avance en energías renovables",
      description: "Nuevo proyecto regional para impulsar energías limpias.",
      category: "TECNOLOGÍA",
      date: "11 Oct 2025",
      image: "/political-debate-panel.jpg",
      country: "Chile",
    },
    {
      id: "f7",
    ...
    {
      id: "f9",
      title: "Educación: nuevas políticas educativas",
      description: "Gobierno anuncia reformas para mejorar la calidad educativa.",
      category: "EDUCACIÓN",
      date: "13 Oct 2025",
    ...
    {
      id: "f10",
      title: "Sociedad: festival internacional de cine",
      description: "Se inaugura el festival con una selección de cine independiente.",
      category: "SOCIEDAD",
      date: "13 Oct 2025",
      image: "/documentary-filming.png",
    ...
    {
      id: "f11",
      title: "Deporte: festival internacional de cine",
      description: "Se inaugura campeonato con una selección de cine independiente.",
      category: "DEPORTES",
      date: "13 Oct 2025",
  ...
  // Resetea el index si cambia la búsqueda para no quedar fuera de rango
  useEffect(() => {
    setFeaturedIndex(0)
  }, [searchQuery])

  // Asegura índice válido cuando cambia el conjunto de slides activas
  useEffect(() => {
    if (activeFeaturedSlides.length === 0) {
      setFeaturedIndex(0)
    } else {
      setFeaturedIndex((i) => (i >= activeFeaturedSlides.length ? 0 : i))
    }
  }, [activeFeaturedSlides.length])

  useEffect(() => {
    // Solo iniciar intervalo si hay slides
    if (activeFeaturedSlides.length === 0) return

    const iv = setInterval(() => {
      if (!featuredAutoPaused) setFeaturedIndex((i) => (i + 1) % activeFeaturedSlides.length)
    }, 3000)
    return () => clearInterval(iv)
  }, [activeFeaturedSlides.length, featuredAutoPaused])
  ...
  // clave establecida por longitudes para evitar reiniciar intervalos en cada render
  const rowLengthsKey = rowData.map((r) => r.length).join(",")

  useEffect(() => {
    type IntervalHandle = ReturnType<typeof window.setInterval>
    // crear timers para cada fila
    const timers: IntervalHandle[] = rowData.map((_, idx) =>
      // TypeScript en browser devuelve number
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      setInterval(() => {
        setRowIndexes((prev) => {
          const slides = Math.max(1, Math.ceil(rowData[idx].length / rowGroupSizes[idx]))
          const next = (prev[idx] + 1) % slides
          const copy = [...prev]
          copy[idx] = next
          return copy
        })
    ...
    // dependemos solo de cambios en la longitud de las filas y tamaños de grupo
  }, [rowLengthsKey, rowGroupSizes])
  ...
  // Variable para obtener la URL actual (para los botones de compartir)
